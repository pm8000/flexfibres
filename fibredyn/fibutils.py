...
def fun_CrankNicolson(z, c, n, n_plus, dt, test='notest', Fz_dyn=0):
    ...
    if test == "linstat":
        ...
        c._gfun[3*n_plus+2] = func_gfun_0(c.m[0], c._G[0], c._F[1], c._Q[1])[2] +\
                                    (1 / c.m[0]) * config.params.fib_BC_Fz
    elif test == "lindyn":
        c._gfun[3*n_plus] = func_gfun_0(c.m[0], c._G[0], c._F[1], c._Q[1])[0]
        c._gfun[3*n_plus+1] = func_gfun_0(c.m[0], c._G[0], c._F[1], c._Q[1])[1]
        c._gfun[3*n_plus+2] = func_gfun_0(c.m[0], c._G[0], c._F[1], c._Q[1])[2] +\
        ...
        c._gfun[3*n_plus] = 0
        c._gfun[3*n_plus+1] = 0
        c._gfun[3*n_plus+2] = 0
    else:
        c._gfun[3*n_plus] = func_gfun_0(c.m[0], c._G[0], c._F[1], c._Q[1])[0]
        c._gfun[3*n_plus+1] = func_gfun_0(c.m[0], c._G[0], c._F[1], c._Q[1])[1]
        c._gfun[3*n_plus+2] = func_gfun_0(c.m[0], c._G[0], c._F[1], c._Q[1])[2]
...
def advance_fibre(c, n, n_plus, dt, test='notest', Fz_dyn=0):
    ...
    solution = root(fun_CrankNicolson, c.z_old, (c, n, n_plus, dt, test, Fz_dyn),
                    method='hybr', options={'xtol': 1.49012e-08, 'maxfev': 1000}) # a bit faster
    #solution = root(fun_CrankNicolson, c.z_old, (c, n, n_plus, dt, test, Fz_dyn),
    ...
    #print("solution is instance of OptimizeResult: {}".format(isinstance(solution, OptimizeResult)))
    #print(type(solution))
    #print(solution.keys())
    print("\nNumber of objective function evauations: ", solution.nfev)
    #print(solution)
    ...
    if test == "linstat":
        c.gfun_new[3*n_plus : 3*n_plus + 3] = func_gfun_0(
            ...
            np.array([0, 0, (1 / c.m[0]) * config.params.fib_BC_Fz])
                                ...
                                #c.gfun_new[3*n_plus : 3*n_plus + 3] = np.array([0, 0, func_gfun_0(
