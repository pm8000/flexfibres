...
# Distributed memory environment, MPI implementaition:
comm = MPI.COMM_WORLD
...
# NOTE: We will take the function update() of Isotropic.py and overload it, so it will be in the solver NS.
def update(context):
...
# NOTE: overloaded by the selected convection function
def conv(*args):
